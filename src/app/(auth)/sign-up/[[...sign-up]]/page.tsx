...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    setError("");
    try {
      await signUp.sso({ strategy: "oauth_google", redirectCallbackUrl: "/sso-callback", redirectUrl: "/" });
    } catch (err: unknown) {
      setError("Er ging iets mis met Google registratie.");
    }
  };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (signUpError) {
      ...
      if (code.includes("password")) {
        setError("Wachtwoord voldoet niet aan de vereisten. Gebruik minimaal 8 tekens.");
      } else if (code.includes("taken") || code.includes("exists")) {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (signUp.status === "complete") {
      ...
      setError("Verificatie kon niet worden voltooid. Probeer het opnieuw.");
    }
  };
        ...
        <div className="rounded-[0.25rem] border border-[#2a5a5a]/50 bg-[#163838]/50 p-8 backdrop-blur-sm">
          <div className="mb-6 flex flex-col items-center gap-3">
            ...
            <p className="text-center text-sm text-[#EAE3DF]/60">
              ...
              <span className="font-medium text-[#EAE3DF]">{email}</span>
            </p>
          </div>
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-[#EAE3DF]">Verificatiecode</label>
              <input
                type="text"
                value={code}
                ...
                required
                autoFocus
                className="h-10 w-full rounded-[0.25rem] border border-[#2a5a5a] bg-[#1a3a3a] px-3 text-center text-lg tracking-widest text-[#EAE3DF] placeholder:text-[#EAE3DF]/30 focus:border-[#14AF52] focus:outline-none focus:ring-1 focus:ring-[#14AF52]/50"
              />
            </div>
            ...
            >
              {isSubmitting ? <Loader2 className="size-4 animate-spin" /> : "Verifiëren"}
            </button>
          </form>
          ...
          <button
            ...
            className="mt-4 w-full text-center text-sm text-[#EAE3DF]/60 transition-colors hover:text-[#EAE3DF]"
          >
    ...
    <div className="w-full max-w-[420px] px-4">
      <div className="rounded-[0.25rem] border border-[#2a5a5a] bg-[#163838] p-8">
        <div className="mb-6 flex flex-col items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-[#EAE3DF]">Registreren</h1>
          <p className="text-sm text-[#EAE3DF]/60">om verder te gaan naar VastVooruit Portaal</p>
        </div>
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-[#EAE3DF]">E-mailadres</label>
            <input
              type="email"
              value={email}
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-[#EAE3DF]">Wachtwoord</label>
          ...
          <button
            ...
            className="flex h-10 w-full items-center justify-center rounded-[0.25rem] bg-[#14AF52] text-sm font-medium text-white transition-colors hover:bg-[#12994a] disabled:opacity-50"
          >
            {isSubmitting ? <Loader2 className="size-4 animate-spin" /> : "Doorgaan"}
          </button>
        </form>

        <p className="mt-5 text-center text-sm text-[#EAE3DF]/60">
          Al een account?{" "}
          <Link href="/sign-in" className="text-[#14AF52] transition-colors hover:text-[#14AF52]/80">
            Inloggen
          </Link>
        </p>
      </div>
